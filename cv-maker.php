...
function cv_maker_enqueue_scripts() {
    wp_enqueue_style('cv-maker-style', CV_MAKER_URL . 'assets/css/style.css');
    wp_enqueue_script('cv-maker-script', CV_MAKER_URL . 'assets/js/script.js', array('jquery'), null, true);
}
...
function cv_maker_form_shortcode() {
    ob_start();
    include CV_MAKER_PATH . 'templates/form-template.php';
    return ob_get_clean();
}
